    ...
    def __init__(
        ...
        for agent in self.agents:
            ...
            self.observation_spaces.append(
                CentStateObservationSpace((obs_dim, ), (obs_dim, )))
            agent.action.c = np.zeros(self.world.dim_c)
        # rendering
        assert shared_viewer
    ...
    def step(self, action_n):
        ...
        for i, agent in enumerate(self.agents):
            ...
            info_n.append({
                **env_info,
                'episode':
                dict(l=self.current_step, r=self.current_return[i]),
            })
        ...
        return (CentStateObservation(np.array(obs_n, dtype=np.float32),
                                     np.array(obs_n,
                                              dtype=np.float32)), reward_n,
                np.array(done_n, dtype=np.uint8), info_n)

    def reset(self):
        self.current_step = 0
        self.current_return = [0 for _ in range(self.num_agents)]
        # reset world
        ...
        return CentStateObservation(np.array(obs_n, dtype=np.float32),
                                    np.array(obs_n, dtype=np.float32))
