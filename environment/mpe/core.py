    ...
    def apply_action_force(self, p_force):
        ...
        for i, agent in enumerate(self.agents):
            if agent.movable:
                ...
                ) * agent.u_noise if agent.u_noise else 0.0
                # force = mass * a * action + n
                p_force[i] = (agent.mass *
                              agent.accel if agent.accel is not None else
                              agent.mass) * agent.action.u + noise
        return p_force
