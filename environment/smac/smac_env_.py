...
from environment.smac.smac_maps import get_map_params
...
class StarCraft2Env(MultiAgentEnv):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        concat_obs_as_state : bool, optional
        ...
        self.concat_obs_as_state = concat_obs_as_state
        ...
        self._seed = np.random.randint(0, int(1e9)) if seed is None else seed
        ...
        if self.use_stacked_frames:
            self.stacked_local_obs = np.zeros(
                (self.n_agents, self.stacked_frames,
            ...
            self.pathing_grid = np.invert(
                np.flip(np.transpose(
                    np.array(list(map_info.pathing_grid.data),
                             dtype=np.uint8).reshape(self.map_x, self.map_y)),
                        axis=1))
    ...
    def reset(self):
        ...
        Returns initial observations and states.
        """
        self._episode_steps = self._episode_return = 0
        ...
        # obs_mask and state_mask will be used in an attention model
        if self.use_state_agent:
            global_state, state_mask = map(
                np.stack,
                zip(*[
        ...
        local_obs, obs_mask = map(np.stack, self.get_obs())

        if self.use_stacked_frames:
            self.stacked_local_obs = np.roll(self.stacked_local_obs, 1, axis=1)
            self.stacked_global_state = np.roll(self.stacked_global_state,
                                                1,
                                                axis=1)
        ...
        return local_obs, global_state, np.array(
            available_actions, dtype=np.uint8), obs_mask, state_mask
    ...
    def step(self, actions):
        ...
        dones = np.zeros((self.n_agents, 1), dtype=np.uint8)
        ...
        try:
            ...
            if self.use_stacked_frames:
                self.stacked_local_obs = np.roll(self.stacked_local_obs,
                                                 1,
                                                 axis=1)
                self.stacked_global_state = np.roll(self.stacked_global_state,
                                                    1,
            ...
            return local_obs, global_state, rewards, dones, infos, np.array(
                available_actions, dtype=np.uint8), obs_mask, state_mask
    ...
    def get_agent_action_heuristic(self, a_id, action):
        ...
        if unit.unit_type == self.medivac_id:
            if (target is None or self.agents[target].health == 0
                    or self.agents[target].health
                    == self.agents[target].health_max):
    ...
    def get_obs_agent(self, agent_id):
        ...
        enemy_mask = np.zeros(enemy_feats_dim[0], dtype=np.uint8)
        ally_feats = np.zeros(ally_feats_dim, dtype=np.float32)
        ally_mask = np.zeros(ally_feats_dim[0], dtype=np.uint8)
        ...
        if unit.health > 0:  # otherwise dead, return all zeros
            ...
            if self.obs_pathing_grid:
                move_feats[ind:ind +
                           self.n_obs_pathing] = self.get_surrounding_pathing(
                               unit)
            ...
            # Enemy features
            assert len(self.enemies) == enemy_mask.shape[0], (len(
                self.enemies), enemy_mask.shape)
            for e_id, e_unit in self.enemies.items():
                e_x = e_unit.pos.x
                e_y = e_unit.pos.y
                dist = self.distance(x, y, e_x, e_y)

                enemy_mask[e_id] = (dist < sight_range and e_unit.health > 0)
                if (dist < sight_range
                        and e_unit.health > 0):  # visible and alive
                    # Sight range > shoot range
                    enemy_feats[e_id,
                                0] = avail_actions[self.n_actions_no_attack +
                                                   e_id]  # available
                    enemy_feats[e_id, 1] = dist / sight_range  # distance
                    enemy_feats[e_id,
                                2] = (e_x - x) / sight_range  # relative X
                    enemy_feats[e_id,
                                3] = (e_y - y) / sight_range  # relative Y

                    ind = 4
                    if self.obs_all_health:
                        enemy_feats[e_id,
                                    ind] = (e_unit.health / e_unit.health_max
                                            )  # health
                        ind += 1
                        if self.shield_bits_enemy > 0:
                            max_shield = self.unit_max_shield(e_unit)
                            enemy_feats[e_id,
                                        ind] = (e_unit.shield / max_shield
                                                )  # shield
                            ind += 1
            ...
            assert self.n_agents - 1 == ally_mask.shape[0], (self.n_agents - 1,
                                                             ally_mask.shape)
            for i, al_id in enumerate(al_ids):
                ...
                ally_mask[i] = (dist < sight_range and al_unit.health > 0)
                if (dist < sight_range
                        and al_unit.health > 0):  # visible and alive
                    ...
                    if self.obs_all_health:
                        ally_feats[i,
                                   ind] = (al_unit.health / al_unit.health_max
                                           )  # health
                        ind += 1
        ...
        agent_obs = np.concatenate(
            (ally_feats.flatten(), enemy_feats.flatten(), move_feats.flatten(),
             own_feats.flatten()))
        mask = np.concatenate((ally_mask, enemy_mask, np.ones(1,
                                                              dtype=np.uint8)))
        ...
        if self.obs_agent_id:
            ...
            agent_obs = np.concatenate(
                (ally_feats.flatten(), enemy_feats.flatten(),
                 move_feats.flatten(), own_feats.flatten(),
    ...
    def get_obs(self):
        ...
        during decentralised execution.
        """
        agents_obs, agent_mask = zip(
            *[self.get_obs_agent(i) for i in range(self.n_agents)])
        return agents_obs, agent_mask

    def get_state(self, agent_id=-1):
        """Returns the global state.
        NOTE: This functon should not be used during decentralised execution.
        """
        if self.concat_obs_as_state:
            agent_obs, agent_mask = self.get_obs()
            obs_concat = np.concatenate(agent_obs, axis=0).astype(np.float32)
            obs_mask = np.concatenate(agent_mask, axis=0).astype(np.uint8)
            return obs_concat, obs_mask
        ...
        ally_mask = np.zeros(self.n_agents, dtype=np.uint8)
        enemy_state = np.zeros((self.n_enemies, nf_en), dtype=np.float32)
        enemy_mask = np.zeros(self.n_enemies, dtype=np.uint8)
        ...
        if (self.use_mustalive and unit.health > 0) or (
                not self.use_mustalive):  # or else all zeros
            # Movement features
            for m in range(self.n_actions_move):
            ...
            assert len(self.agents) == ally_mask.shape[0], (len(self.agents),
                                                            ally_mask.shape)
            for al_id, al_unit in self.agents.items():
                ally_mask[al_id] = (al_unit.health > 0)
                if al_unit.health > 0:
                    al_x = al_unit.pos.x
                    al_y = al_unit.pos.y
                    ...
                    if (self.map_type == "MMM"
                        ...
                        ally_state[al_id,
                                   1] = al_unit.energy / max_cd  # energy
                    else:
                        ally_state[al_id,
                                   1] = (al_unit.weapon_cooldown / max_cd
                                         )  # cooldown
                    ...
                    if self.add_center_xy:
                        ...
                        ally_state[al_id, ind + 1] = (
                            al_y - center_y) / self.max_distance_y  # center Y
                        ind += 2
                    ...
                    if unit.health > 0:
                        ...
                        if self.add_distance_state:
                            ally_state[al_id,
                                       ind] = dist / sight_range  # distance
                            ind += 1
                        if self.add_xy_state:
                            ally_state[al_id,
                                       ind] = (al_x -
                                               x) / sight_range  # relative X
                            ally_state[al_id, ind +
                                       1] = (al_y -
                                             y) / sight_range  # relative Y
                            ind += 2
                        if self.add_visible_state:
                            if dist < sight_range:
            ...
            assert self.n_enemies == enemy_mask.shape[0], (self.n_enemies,
                                                           enemy_mask.shape)
            for e_id, e_unit in self.enemies.items():
                enemy_mask[e_id] = (e_unit.health > 0)
                if e_unit.health > 0:
                    e_x = e_unit.pos.x
                    e_y = e_unit.pos.y
                    dist = self.distance(x, y, e_x, e_y)
                    ...
                    if self.add_center_xy:
                        ...
                        enemy_state[e_id, ind + 1] = (
                            e_y - center_y) / self.max_distance_y  # center Y
                        ind += 2
                    ...
                    if unit.health > 0:
                        ...
                        if self.add_enemy_action_state:
                            enemy_state[e_id, ind] = avail_actions[
                                self.n_actions_no_attack + e_id]  # available

        state = np.append(ally_state.flatten(), enemy_state.flatten())
        mask = np.append(ally_mask, enemy_mask)

        if self.add_move_state:
            state = np.append(state, move_state.flatten())

        if self.add_local_obs:
            state = np.append(state, self.get_obs_agent(agent_id)[0].flatten())
    ...
    def get_cent_state(self):
        ...
        state = ally_state.flatten().astype(dtype=np.float32)
    ...
    def get_state_agent(self, agent_id):
        ...
        if (self.use_mustalive and unit.health > 0) or (
                not self.use_mustalive):  # otherwise dead, return all zeros
            x = unit.pos.x
            y = unit.pos.y
            sight_range = self.unit_sight_range(agent_id)
            ...
            # Enemy features
            assert len(self.enemies) == enemy_mask.shape[0], (len(
                self.enemies), enemy_mask.shape[0])
            for e_id, e_unit in self.enemies.items():
                e_x = e_unit.pos.x
                e_y = e_unit.pos.y
                dist = self.distance(x, y, e_x, e_y)
            ...
            assert len(al_ids) == ally_mask.shape[0], (len(al_ids),
                                                       ally_mask.shape)
            for i, al_id in enumerate(al_ids):
                ...
                if al_unit.health > 0:  # visible and alive
                    if unit.health > 0:
                        ...
                        ally_feats[i, 1] = dist / sight_range  # distance
                        ally_feats[i,
                                   2] = (al_x - x) / sight_range  # relative X
                        ally_feats[i,
                                   3] = (al_y - y) / sight_range  # relative Y
            ...
            if self.add_center_xy:
                own_feats[ind] = (x -
                                  center_x) / self.max_distance_x  # center X
                own_feats[ind +
                          1] = (y - center_y) / self.max_distance_y  # center Y
                ind += 2
        ...
        state = np.concatenate((ally_feats.flatten(), enemy_feats.flatten(),
                                move_feats.flatten(), own_feats.flatten()))
        mask = np.concatenate((ally_mask, enemy_mask, np.ones(1,
                                                              dtype=np.uint8)))
    ...
    def get_obs_size(self, agent_specific=False):
        """Returns the size of the observation."""
        ...
        if agent_specific and self.use_stacked_frames:
            raise NotImplementedError(
                "agent_specific and use_stacked_frames can't be True at the same time!"
            )

        if agent_specific:
            return collections.OrderedDict(
                obs_allies=(n_allies, n_ally_feats),
                obs_enemies=(n_enemies, n_enemy_feats),
                obs_move=(1, move_feats),
                obs_self=(own_feats + agent_id_feats + timestep_feats, ),
                obs_mask=(n_allies + n_enemies + 1, ))
        else:
            return (all_feats, ) if not self.use_stacked_frames else (
                self.stacked_frames, all_feats)

    def get_state_size(self, agent_specific=False):
        """Returns the size of the global state."""
        if self.concat_obs_as_state:
            if agent_specific:
                raise NotImplementedError(
                    "agent_specific and concat_obs_as_state can't be True at the same time!"
                )
        ...
        if self.use_state_agent:
            ...
            if agent_specific:
                return collections.OrderedDict(
                    state_allies=(n_allies, n_ally_feats),
                    state_enemies=(n_enemies, n_enemy_feats),
                    state_move=(1, move_feats),
                    state_self=(own_feats + timestep_feats + agent_id_feats, ),
                    state_mask=(n_allies + n_enemies + 1, ))
            else:
                return (self.stacked_frames,
                        all_feats) if self.use_stacked_frames else (
                            all_feats, )
        ...
        if agent_specific:
            own_feat_size = move_state + obs_agent_size + timestep_state + agent_id_feats
            if own_feat_size > 0:
                return collections.OrderedDict(
                    state_allies=(self.n_agents, nf_al),
                    state_enemies=(self.n_enemies, nf_en),
                    state_self=(own_feat_size, ),
                    state_mask=(self.n_agents + self.n_enemies, ))
            else:
                return collections.OrderedDict(
                    state_allies=(self.n_agents, nf_al),
                    state_enemies=(self.n_enemies, nf_en),
            ...
            return (size, ) if not self.use_stacked_frames else (
                self.stacked_frames, size)
    ...
    def get_visibility_matrix(self):
        ...
        """
        arr = np.zeros((self.n_agents, self.n_agents + self.n_enemies),
                       dtype=np.uint8)
    ...
    def init_units(self):
        ...
        while True:
            ...
            if self._episode_count == 0:
                min_unit_type = min(unit.unit_type
