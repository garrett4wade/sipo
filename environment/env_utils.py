...
from utils.namedarray import namedarray, array_like
...
@namedarray
class CentStateObservation(BasicObservation):
    cent_state: Optional[torch.Tensor] = None


class CentStateObservationSpace(BasicObservationSpace):

    def __init__(self, shape, cent_state_shape=None):
        super().__init__(shape)
        self._cent_state_shape = cent_state_shape

    def sample(self):
        if self._cent_state_shape is not None:
            return CentStateObservation(super().sample().obs,
                                        torch.randn(*self._cent_state_shape))
        else:
            return CentStateObservation(super().sample().obs)


class ObservationSpaceFromExample:

    def __init__(self, example: env_base.Observation):
        self._example = array_like(example)

    def sample(self):
        return array_like(self._example)
