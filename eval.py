...
def main(args):
    ...
    if all_args.eval or all_args.render:
        for i, policy in enumerate(archive_policies):
            eval_storages = []
            critic_dim = get_critic_dim(all_args.algo, i)
            for _ in range(all_args.num_env_splits):
                ...
                eval_storage = recursive_apply(
                    eval_storage,
                    lambda x: x.repeat(
                        all_args.eval_episode_length + 1, all_args.
                        num_eval_envs // all_args.num_env_splits, num_agents,
                        *((1, ) * len(x.shape))).share_memory_(),
                )
                eval_storage.step = torch.tensor(
            ...
            eval_env_ctrls = [[
                EnvironmentControl(mp.Semaphore(0), mp.Semaphore(0),
                                   mp.Event(), mp.Event(), mp.Event())
            ...
            # start worker
            video_file = all_args.video_file.split('.')
            video_file[-2] += f"_iter{i}"
    ...
    for iteration in range(len(all_args.archive_policy_dirs),
        ...
        runner = SharedRunner(iteration,
                              ...
                              eval_info_queue,
                              device,
                              run_dir=run_dir / f"iter{iteration}")
