...
class CatSelfEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        for k in self.others_keys:
            if 'mask' not in k:
                setattr(
                    self, k + '_fc',
                    get_layer(others_shape_dict[k][-1] + self_dim,
                              d_embedding))
    ...
    def forward(self, self_vec, **inputs):
        ...
        other_embeddings = torch.cat([self_embedding.unsqueeze(-2)] + other_embeddings, dim=-2)
...
def ScaledDotProductAttention(q, k, v, mask=None, dropout=None):
    ...
    scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(
        d_k)  # (NUM_HEADS, NUM_TOKENS_1, NUM_TOKENS_2)
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def __init__(self, input_dim, d_attn, n_heads, dropout=0.0):
        ...
        if not d_attn % n_heads == 0:
            raise ValueError(
                'Attention model dimension is not a multiple of the number of attention heads.'
            )
    ...
    def forward(self, x, mask):
        ...
        q = self.q_linear(x).view(*x.shape[:-1], self.n_heads,
                                  self.d_head).transpose(-2, -3)
        k = self.k_linear(x).view(*x.shape[:-1], self.n_heads,
                                  self.d_head).transpose(-2, -3)
        v = self.v_linear(x).view(*x.shape[:-1], self.n_heads,
                                  self.d_head).transpose(-2, -3)
        ...
        return scores.transpose(-2,
                                -3).contiguous().view(*x.shape[:-1],
                                                      self.d_attn)
