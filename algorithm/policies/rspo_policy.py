    ...
    def __init__(
        ...
        **kwargs,
        ...
        if isinstance(action_space, gym.spaces.Discrete):
            ...
            self._action_dim = action_dim = action_space.n
            self.action_out = get_initialized_linear(hidden_dim, action_dim,
                                                     act_layer_gain)
        elif isinstance(action_space, gym.spaces.Box):
            self.action_type = 'continuous'
            self._action_dim = action_dim = action_space.shape[0]
...
class Critic(nn.Module):
    ...
    def __init__(self, *args, **kwargs):
        super().__init__()
        if kwargs.get('critic_dim', 1) == 1 or kwargs.get(
                'share_critic_backbone', False):
            self._share_backbone = True
            self.net = _Critic(*args, **kwargs)
        else:
            num_critics = copy.deepcopy(kwargs['critic_dim'])
            kwargs['critic_dim'] = 1
            self._share_backbone = False
            self.nets = nn.ModuleList(
                [_Critic(*args, **kwargs) for _ in range(num_critics)])

    @property
    def v_out(self):
        if self._share_backbone:
            return self.net.v_out
        else:
            return [net.v_out for net in self.nets]

    def forward(self, state, rnn_states, masks):
        if self._share_backbone:
            return self.net(state, rnn_states, masks)
        if rnn_states is not None:
            rnn_states = torch.split(rnn_states,
                                     rnn_states.shape[-1] // len(self.nets),
                                     dim=-1)
        else:
            rnn_states = [None for _ in range(len(self.nets))]
        rnn_states_ = []
        values = []
        for rnn_state, net in zip(rnn_states, self.nets):
            value, rnn_state_ = net(state, rnn_state, masks)
            rnn_states_.append(rnn_state)
            values.append(value)
        if rnn_states_[0] is None:
            rnn_states_ = None
        else:
            rnn_states_ = torch.cat(rnn_states_, -1)
        return torch.cat(values, -1), rnn_states_
...
class ActorCriticPolicyStateSpace:
    ...
    def __init__(self,
        ...
        self.share_critic_backbone = share_critic_backbone

    def sample(self):
        if self.share_critic_backbone:
            return PolicyState(
            ...
            return PolicyState(
                ...
                torch.zeros(self.num_rnn_layers,
                            self.hidden_dim * self.critic_dim))
...
class ActorCriticPolicy:
    ...
    def __init__(
        ...
        critic_dim=1,
        share_critic_backbone=True,
        use_reward_predictor=True,
        **kwargs,
    ):
        self._popart = popart
        self._share_critic_backbone = (share_critic_backbone
                                       or (critic_dim == 1))
        self._device = device
        ...
        self.critic = Critic(state_dim,
                             ...
                             share_critic_backbone=(share_critic_backbone
                                                    or (critic_dim == 1)),
                             **kwargs).to(device)

        self.policy_state_space = ActorCriticPolicyStateSpace(
            num_rnn_layers, hidden_dim, critic_dim, share_critic_backbone)
        ...
        if use_reward_predictor:
            ...
            self.r_predictor = nn.Sequential(
                modules.mlp([obs_dim + self.act_dim, hidden_dim, hidden_dim],
                            layernorm=False), nn.Linear(hidden_dim,
                                                        1)).to(device)

        self._version = 0

    def get_reward_prediction(self, obs, action):
        assert self.use_reward_predictor
        if self._action_type == 'discrete':
            action = nn.functional.one_hot(
                action.squeeze(-1).long(), self.act_dim).float()
        elif self._action_type == 'continuous':
            pass
        else:
            raise NotImplementedError()

        return self.r_predictor(torch.cat([obs, action], -1))
    ...
    def normalize_value(self, x):
        if not self._share_critic_backbone:
            normalized_x = []
            for i, head in enumerate(self.popart_head):
                normalized_x.append(head.normalize(x[..., i:i + 1]))
            return torch.cat(normalized_x, -1)
    ...
    def denormalize_value(self, x):
        if not self._share_critic_backbone:
            denormalized_x = []
            for i, head in enumerate(self.popart_head):
                denormalized_x.append(head.denormalize(x[..., i:i + 1]))
            return torch.cat(denormalized_x, -1)
    ...
    def analyze(self, sample: SampleBatch, target="ppo", **kwargs):
        ...
        actor_output, _ = self.actor(
            ...
            getattr(sample.obs, 'available_action', None))
    ...
    def rollout(self, requests: RolloutRequest, deterministic):
        ...
        actor_output, actor_hx = self.actor(
            ...
            getattr(requests.obs, "available_action", None))
...
register("rspo", ActorCriticPolicy)