...
class RSPO:
    ...
    def __init__(self, args, policy, **kwargs):
        ...
        if self.args.use_reward_predictor:
            self.reward_predictor_optimizer = torch.optim.Adam(
                self.policy.r_predictor.parameters(),
                lr=args.critic_lr,
                eps=args.opti_eps,
                weight_decay=args.weight_decay,
            )
    ...
    def cal_value_loss(self, values, value_preds_batch, return_batch,
                       active_masks_batch, interpolation_masks):
        ...
        value_loss_original = (value_loss_original * interpolation_masks).sum(
            -1, keepdim=True)
    ...
    def ppo_update(self, sample: SampleBatch, update_actor=True):
        ...
        policy_loss = (-torch.min(surr1, surr2) *
                       sample.interpolation_masks).sum(-1, keepdim=True)

        if self._use_policy_active_masks:
            policy_loss = (policy_loss * sample.active_masks
        ...
        if self.args.use_reward_predictor:
            reward_prediction = self.policy.get_reward_prediction(
                sample.obs.obs, sample.actions)
            r_loss = (reward_prediction - sample.rewards).pow(2)
            r_loss = (r_loss *
        ...
        return value_loss, critic_grad_norm, policy_loss, dist_entropy, actor_grad_norm, imp_weights, r_loss
        ...
        for _ in range(self.ppo_epoch):
            ...
            for sample in data_generator:
                (value_loss, critic_grad_norm, policy_loss, dist_entropy,
                 ...
                 r_loss) = self.ppo_update(sample, update_actor=update_actor)
                ...
                if r_loss is not None:
                    train_info['reward_pred_loss'] = r_loss
